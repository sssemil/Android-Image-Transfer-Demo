        ...
        if (event.getAction() == MotionEvent.ACTION_DOWN) {
            ...
            if (mEnabled) {
                if (rawX > mRgbRegion.left && rawX < mRgbRegion.right && rawY > mRgbRegion.top && rawY < mRgbRegion.bottom) {
                    ...
                    mColorHue = (float) Math.atan2(normalizedY, normalizedX);
                    mColorHue = (mColorHue + (float) Math.PI) / (2.0f * (float) Math.PI);
                    mColorIntensity = (float) Math.sqrt(Math.pow(normalizedX, 2.0) + Math.pow(normalizedY, 2.0));
                    ...
                    Log.d("RgbDemo", mColorHue + ", " + mColorIntensity);
                    return true;
                }
            }
        }
